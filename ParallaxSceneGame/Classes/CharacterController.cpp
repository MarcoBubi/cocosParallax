...
void CharacterController::setup() {
    auto listener = cocos2d::EventListenerCustom::create(GameEvents::EVENT_UI_BUTTON_JUMP_PRESSED, CC_CALLBACK_1(CharacterController::handleJumpEvent, this));
...
void CharacterController::handleJumpEvent(cocos2d::EventCustom* event) {
    if (_characterNode && !_characterNode->getNumberOfRunningActions()) {
        // refactor this values to be calculated first
        auto jumpAction = cocos2d::JumpBy::create(1.0, cocos2d::Vec2(0, 0), 1000, 1);
