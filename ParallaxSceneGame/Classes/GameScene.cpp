...
bool GameScene::init() {
    ...
    auto sprite = cocos2d::Sprite::create("textures/character.png");

    // Assuming you have variables for the screen's width and height
    sprite->setPosition(cocos2d::Vec2(1920 / 2, (sprite->getContentSize().height / 2) - cocos2d::Director::getInstance()->getVisibleOrigin().y));
    auto a = this->getContentSize();
    ...
    float y = origin.y;                                                  // y for the center of the screen
    ...
    _parallaxController->addSpriteToLayerWithAutoZOrder("background_4_mountains.png", ParallaxController::ParallaxLayer::Background, cocos2d::Vec2(.35f, .0f), cocos2d::Vec2(x, y));

    // changed this clouds ratio just for the sake of dynamics and testability, not sure if it breaks the parallax principle.
    _parallaxController->addSpriteToLayerWithAutoZOrder("background_5_clouds.png", ParallaxController::ParallaxLayer::Background, cocos2d::Vec2(2.4f, .0f), cocos2d::Vec2(x, y));
    _parallaxController->addSpriteToLayerWithAutoZOrder("background_6_clouds.png", ParallaxController::ParallaxLayer::Background, cocos2d::Vec2(.45f, .0f), cocos2d::Vec2(x, y));
